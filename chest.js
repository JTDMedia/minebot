...
if (process.argv.length < 4 || process.argv.length > 5) {
  console.log('Usage : node chest.js <host> <port> <master>')
  process.exit(1)
}
...
const bot = mineflayer.createBot({
  ...
  port: parseInt(process.argv[3]),              // set if you need a port that isn't 25565
  version: '1.21.4'
})
...
bot.on('chat', (username, message) => {
  ...
  if (username !== process.argv[4]) return
  switch (true) {
...
bot.once('spawn', () => {
  ...
  console.log("Viewer was enabled on http://localhost:3007")
})
